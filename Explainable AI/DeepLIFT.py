...
attributions = deeplift.attribute(input_tensor, target=predicted_class.item())
print("Shape of attributions:", attributions.shape)

deeplift_map = attributions.squeeze().detach().numpy()
original_volume = preprocessed_subject['t1']['data'].squeeze().numpy()

alpha = 0.3
overlay_5 = (1 - alpha) * original_volume + alpha * deeplift_map
...
plt.subplot(1, 2, 2)
plt.imshow(overlay_5[:, :, 86], cmap='hot')
plt.colorbar()
plt.title('DeepLIFT', y=-0.15)
plt.tight_layout()
plt.show()