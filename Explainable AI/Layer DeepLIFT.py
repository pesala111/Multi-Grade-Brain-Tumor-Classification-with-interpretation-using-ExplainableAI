...
# Define a baseline
#baseline = torch.zeros_like(input_tensor).to(device)  baselines=baseline,

# Apply Layer DeepLIFT
layer_deeplift = captum.attr.LayerDeepLift(model, target_layer)
attributions = layer_deeplift.attribute(input_tensor, target=predicted_class.item())

print("Shape of attributions:", attributions.shape)

upsampled_attributions = captum.attr.LayerAttribution.interpolate(attributions,(240, 240, 155),interpolate_mode='trilinear')
print("Shape of upsampled attributions:", upsampled_attributions.shape)

# Layer DeepLift processing
Layer_DeepLift_map = upsampled_attributions.squeeze().detach().numpy()
original_volume = preprocessed_subject['t1']['data'].squeeze().numpy()

alpha = 0.3
overlay = (1 - alpha) * original_volume + alpha * Layer_DeepLift_map
