...
# Apply Layer Integrated Gradients
lig = captum.attr.LayerIntegratedGradients(model, target_layer)
attributions = lig.attribute(input_tensor, target=predicted_class.item())

print("Shape of attributions:", attributions.shape)

# Layer Integrated Gradients processing
layer_int_gra_map = attributions.squeeze().detach().numpy()
original_volume = preprocessed_subject['t1']['data'].squeeze().numpy()

alpha = 0.3
overlay = (1 - alpha) * original_volume + alpha * layer_int_gra_map
