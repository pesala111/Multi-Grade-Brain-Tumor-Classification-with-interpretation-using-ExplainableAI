...
# Guided GradCAM
target_layer = model.features[6] # Total 13 layers
guided_grad_cam = captum.attr.GuidedGradCam(model, target_layer)
attributions = guided_grad_cam.attribute(input_tensor, target=predicted_class.item())
print("Shape of attributions:", attributions.shape)


# Guided GradCAM processing
Guided_GradCAM_map = attributions.squeeze().detach().numpy()
original_volume = preprocessed_subject['t1']['data'].squeeze().numpy()


alpha = 0.3
overlay = (1 - alpha) * original_volume + alpha * Guided_GradCAM_map
...
plt.colorbar()
plt.title('Guided GradCAM', y=-0.15)
