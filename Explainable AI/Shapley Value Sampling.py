...
shapley_value_sampling = captum.attr.ShapleyValueSampling(model)
attributions = shapley_value_sampling.attribute(input_tensor, target=predicted_class.item())
print("Shape of attributions:", attributions.shape)


# shapley value sampling processing
shapley_map = attributions.squeeze().detach().numpy()
original_volume = preprocessed_subject['t1']['data'].squeeze().numpy()

alpha = 0.3
overlay = (1 - alpha) * original_volume + alpha * shapley_map
