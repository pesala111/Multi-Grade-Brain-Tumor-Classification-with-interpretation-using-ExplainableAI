...
attributions = integrated_gradients.attribute(input_tensor, target=predicted_class.item(), n_steps=50)
print("Shape of attributions:", attributions.shape)

integrated_gradients_map = attributions.squeeze().numpy()
original_volume = preprocessed_subject['t1']['data'].squeeze().numpy()

alpha = 0.3
overlay = (1 - alpha) * original_volume + alpha * integrated_gradients_map
