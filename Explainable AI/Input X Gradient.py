...
attributions = input_x_gradient.attribute(input_tensor, target=predicted_class.item()).to(device)
print("Shape of attributions:", attributions.shape)

# Input x Gradient processing
input_x_gradient_map = attributions.squeeze().cpu().detach().numpy()
original_volume = preprocessed_subject['t1']['data'].squeeze().numpy()

alpha = 0.3
overlay = (1 - alpha) * original_volume + alpha * input_x_gradient_map
...
plt.colorbar()
plt.title('Input X Gradient', y=-0.15)
