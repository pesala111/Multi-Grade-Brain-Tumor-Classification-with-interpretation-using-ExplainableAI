...
layer_grad_cam = captum.attr.LayerGradCam(model, model.features[4])
attributions = layer_grad_cam.attribute(input_tensor, target=predicted_class.item())
...
# Layer GradCAM processing
Layer_GradCAM_map = upsampled_attributions.squeeze().detach().numpy()
original_volume = preprocessed_subject['t1']['data'].squeeze().numpy()

alpha = 0.3
overlay = (1 - alpha) * original_volume + alpha * Layer_GradCAM_map
...
plt.colorbar()
plt.title('Layer GradCAM', y=-0.15)
