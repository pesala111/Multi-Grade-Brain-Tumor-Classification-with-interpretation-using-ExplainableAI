...
deconvolution = captum.attr.Deconvolution(model)
attributions = deconvolution.attribute(input_tensor, target=predicted_class.item())
print("Shape of attributions:", attributions.shape)

# Deconvolution processing
deconv_map = attributions.squeeze().detach().numpy()
original_volume = preprocessed_subject['t1']['data'].squeeze().numpy()

alpha = 0.3
overlay = (1 - alpha) * original_volume + alpha * deconv_map
...
plt.colorbar()
plt.title('Deconvolution', y=-0.15)
